...
export function AiChatPanel() {
  ...
  const confirmInsert = useCallback(async (zone: "core" | "locked" | "active" | "other") => {
    ...
    if (pendingEdges.length > 0) { const ek = "worldview-edges-" + cur.id; const ex = getJSONSync(ek, [] as any[]); for (const ea of pendingEdges) { const si = idMap.get(ea.sourceTitle), ti = idMap.get(ea.targetTitle); if (si && ti) ex.push({ id: uuid(), source: si, target: ti, type: "straight", style: { stroke: "#94a3b8", strokeWidth: 2 } }); } try { setJSONSync(ek, ex); } catch { } }
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-xl" onClick={e => e.stopPropagation()} style={{ minWidth: 340 }}>
            ...
            <div className="flex flex-col gap-2">
              {[
                ...
                <button key={z.key} onClick={() => confirmInsert(z.key as "core" | "locked" | "active" | "other")}
